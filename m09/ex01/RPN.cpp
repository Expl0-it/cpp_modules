...
bool RPN::compute(std::string line)
{
	...
	for (std::string::iterator i = line.begin(); i != line.end(); i++)
	{
		...
		{
			if (_operands.size() < 2)
				return (std::cerr << "Invalid number of operands before operator" << std::endl, false);
			b = _operands.top();
			_operands.pop();
			a = _operands.top();
			_operands.pop();
			...
			return (std::cerr << "Invalid operator" << std::endl, false);
	}
	if (1 != _operands.size())
		return (std::cerr << "Invalid number of opeartors regarding the operants" << std::endl, false);
